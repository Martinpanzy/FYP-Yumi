...
class kinect_vision:
	...
	def depth_callback(self,data):
		if(self.need_adj == False):
			...
			seg = p.make_segmenter_normals(ksearch=400)
			...
			if len(self.coe) < 5:	
				if(np.isnan(coefficients[0])==False and np.isnan(coefficients[1])==False and np.isnan(coefficients[2])==False and coefficients[0]>0):
					...
					pick = [0.04*self.coe[0], 0.04*self.coe[1], 0.04*self.coe[2]]
	...
	def bbx_callback(self,bx):
		for box in bx.bounding_boxes:
			if(box.Class == 'footwear' or box.Class == 'shoe'):
				...
				if (box_h >= 1.8* box_w):
					self.adr_x = int(box.xmax + 70)
					self.adr_xx = int(box.xmin + 0.14*box_w)
					self.adr_y = int(box.ymax - 0.10*box_h)

					self.adl_x = int(box.xmin - 70)
					self.adl_xx = int(box.xmax - 0.14*box_w)
					self.adl_y = int(box.ymin + 0.15*box_h)
					self.need_adj = True
